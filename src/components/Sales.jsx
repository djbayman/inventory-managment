import { CiSquareMore } from "react-icons/ci";
import productImg from "../assets/depositphotos_2891341-stock-photo-modern-laptop.jpg";
import { Link } from "react-router-dom";
import { FaPlus } from "react-icons/fa6";
import { FiEdit3 } from "react-icons/fi";
import { CgDollar } from "react-icons/cg";
import { useRef, useState } from "react";

const Sales = () => {
  const [toggle, setToggle] = useState(false);
  const actionRef = useRef(null);

  const closeOpenAction = (e) => {
    if (toggle && !actionRef.current?.contains(e.target)) {
      setToggle(false);
    }
  };

  document.addEventListener("mousedown", closeOpenAction);

  return (
    <div className="mx-6">
      <div className="comp-head my-6">
        <h1 className="text-2xl font-bold">Sales</h1>
      </div>
      <div className="border-2 rounded-md border-slate-300 min-h-64 my-6">
        <ul className="p-6 text-gray-500 text-md flex items-center justify-between  border-b-2 border-slate-300 ">
          <li className="w-20 text-center">#</li>
          <li className="w-32 ">Product Name</li>
          <li className="w-32 ">Product Image</li>
          <li className="w-32 ">Product Quantity</li>
          <li className="w-32 ">Product Price</li>
          <li className="w-32 ">Company Name</li>
          <li className="w-32 ">Date</li>
          <li className="w-32 ">Action</li>
        </ul>
        <ul className="p-6 text-gray-500 text-md flex items-center justify-between  border-b-2 border-slate-300 hover:bg-cyan-50">
          <li className="w-20 text-center">1</li>
          <li className="w-32 ">Lap top</li>
          <li className="w-32 ">
            <img src={productImg} alt="" className="w-10 h-10 rounded-full " />
          </li>
          <li className="w-32 ">12</li>
          <li className="w-32 ">$1320</li>
          <li className="w-32 ">Dell</li>
          <li className="w-32 ">29 apr</li>
          <li className="w-32 relative" ref={actionRef}>
            <CiSquareMore
              className=" text-4xl text-cyan-950 cursor-pointer
                "
              onClick={(e) => setToggle(!toggle)}
            />
            <div
              className={
                toggle
                  ? "absolute top-10 right-32 w-24   bg-gray-100 text-black font-semibold rounded-md"
                  : "hidden"
              }
            >
              <span className="flex items-center gap-2 justify-center pb-2 hover:border-x-4 hover:border-cyan-600  transition-colors cursor-pointer">
                <FiEdit3 /> Edit
              </span>
              <span className="flex items-center gap-2 justify-center pt-2 hover:border-x-4 hover:border-emerald-600 transition-colors cursor-pointer">
                <CgDollar /> Sale
              </span>
            </div>
          </li>
        </ul>
      </div>
      <div className=" my-6 ">
        <button className="ms-auto px-3 py-2 border-2 border-cyan-700 bg-cyan-600 hover:bg-cyan-700 transition-colors text-white text-xl font-semibold flex items-center rounded-md gap-3">
          <Link
            to="/sales/addSale"
            className="flex items-center rounded-md gap-3"
          >
            <span>
              <FaPlus />
            </span>
            Add To Sale
          </Link>
        </button>
      </div>
    </div>
  );
};

export default Sales;
